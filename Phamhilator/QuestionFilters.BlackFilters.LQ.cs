	...
	namespace QuestionFilters
	{
		namespace BlackFilters
		{
			public class LQ
			{
				...
				public LQ()
				{
					var data = File.ReadAllLines(DirectoryTools.GetQBlackLQTermsFile());
				...
				public void AddTerm(Regex term)
				{
					...
					File.AppendAllText(DirectoryTools.GetQBlackLQTermsFile(), "\n" + AverageScore + "]" + term);
				}
				...
				public void RemoveTerm(Regex term)
				{
					...
					var data = File.ReadAllLines(DirectoryTools.GetQBlackLQTermsFile()).ToList();
