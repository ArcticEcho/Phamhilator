		...
		namespace WhiteFilters
		{
			public class BadUsername
			{
				...
				public BadUsername()
				{
					...
					var sites = Directory.EnumerateDirectories(DirectoryTools.GetQWhiteBadUsernameTermsDir()).ToArray();
					...
					{
						var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetQWhiteBadUsernameTermsDir(), site, "Terms.txt"));
				...
				{
					var file = Path.Combine(DirectoryTools.GetQWhiteBadUsernameTermsDir(), site, "Terms.txt");

					if (!Terms.ContainsKey(site))
					{
						Terms.Add(site, new Dictionary<Regex, float>());

						Directory.CreateDirectory(Path.Combine(DirectoryTools.GetQWhiteBadUsernameTermsDir(), site));
					}
