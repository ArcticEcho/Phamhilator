		...
		namespace WhiteFilters
		{
			public class Offensive
			{
				...
				public Offensive()
				{
					...
					var sites = Directory.EnumerateDirectories(DirectoryTools.GetQWhiteOffensiveTermsDir()).ToArray();
					...
					{
						var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetQWhiteOffensiveTermsDir(), site, "Terms.txt"));
				...
				{
					var file = Path.Combine(DirectoryTools.GetQWhiteOffensiveTermsDir(), site, "Terms.txt");

					if (!Terms.ContainsKey(site))
					{
						Terms.Add(site, new Dictionary<Regex, float>());

						Directory.CreateDirectory(Path.Combine(DirectoryTools.GetQWhiteOffensiveTermsDir(), site));
					}
