...
namespace Phamhilator.AnswerFilters.White
{
	public class BadUsername
	{
		...
		public BadUsername()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetFilterFile(Filters.AnswerWhiteName)).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteName), site, "Terms.txt"));
		...
		{
			var file = Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteName), site, "Terms.txt");

			if (!Terms.ContainsKey(site))
			{
				Terms.Add(site, new Dictionary<Regex, float>());

				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteName), site));
			}
