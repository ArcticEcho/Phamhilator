		...
		namespace White
		{
			public class Offensive
			{
				...
				public Offensive()
				{
					...
					var sites = Directory.EnumerateDirectories(DirectoryTools.GetAWhiteOffensiveTermsDir()).ToArray();
					...
					{
						var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetAWhiteOffensiveTermsDir(), site, "Terms.txt"));
				...
				{
					var file = Path.Combine(DirectoryTools.GetAWhiteOffensiveTermsDir(), site, "Terms.txt");

					if (!Terms.ContainsKey(site))
					{
						Terms.Add(site, new Dictionary<Regex, float>());

						Directory.CreateDirectory(Path.Combine(DirectoryTools.GetAWhiteOffensiveTermsDir(), site));
					}
