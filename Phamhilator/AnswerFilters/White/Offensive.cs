...
namespace Phamhilator.AnswerFilters.White
{
	public class Offensive
	{
		...
		public Offensive()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetFilterFile(Filters.AnswerWhiteOff)).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteOff), site, "Terms.txt"));
		...
		{
			var file = Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteOff), site, "Terms.txt");

			if (!Terms.ContainsKey(site))
			{
				Terms.Add(site, new Dictionary<Regex, float>());

				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteOff), site));
			}
