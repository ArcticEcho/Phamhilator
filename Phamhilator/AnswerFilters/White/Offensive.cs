...
namespace Phamhilator.AnswerFilters.White
{
	public class Offensive
	{
		...
		public Offensive()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetAWOffTermsDir()).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetAWOffTermsDir(), site, "Terms.txt"));
		...
		public void AddTerm(Regex term, string site, float score = -1)
		{
			...
			if (!Terms.ContainsKey(site))
			{
				...
				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetAWOffTermsDir(), site));
			}
