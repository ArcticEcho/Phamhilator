...
namespace Phamhilator.AnswerFilters.White
{
	public class LQ
	{
		...
		public LQ()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetFilterFile(Filters.AnswerWhiteLQ)).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteLQ), site, "Terms.txt"));
		...
		{
			var file = Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteLQ), site, "Terms.txt");

			if (!Terms.ContainsKey(site))
			{
				Terms.Add(site, new Dictionary<Regex, float>());

				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetFilterFile(Filters.AnswerWhiteLQ), site));
			}
