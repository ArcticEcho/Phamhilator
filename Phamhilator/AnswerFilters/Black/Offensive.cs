	...
	namespace AnswerFilters
	{
		namespace Black
		{
			public class Offensive
			{
				...
				public Offensive()
				{
					var data = File.ReadAllLines(DirectoryTools.GetABlackOffensiveTermsFile());
				...
				public void AddTerm(Regex term)
				{
					...
					File.AppendAllText(DirectoryTools.GetABlackOffensiveTermsFile(), "\n" + AverageScore + "]" + term);
				}
				...
				public void RemoveTerm(Regex term)
				{
					...
					var data = File.ReadAllLines(DirectoryTools.GetABlackOffensiveTermsFile()).ToList();
