...
namespace Phamhilator
{
	...
	{
		public static void PostMessage(string message, Question post = null, PostAnalysis report = null/* int consecutiveMessageCount = 0*/)
		{
		...
		public static void PostMessage(string message, Answer post = null, PostAnalysis report = null/* int consecutiveMessageCount = 0*/)
		{
			if (GlobalInfo.RoomID == 0) { return; }
			...
			while (html.IndexOf(post.Excerpt, StringComparison.Ordinal) == -1)
			{
				if (i >= 5) { return; }
			...
			var id = HTMLScraper.GetMessageIDByReportTitle(html, post.Excerpt);
