...
namespace Phamhilator
{
	public static class PostChecker
	{
		...
		private static readonly Regex lowQuality = new Regex(@"homework|builtwith.com|very difficult|newbie|\bq\b|question|tried|\bif (yes|no)\b|my best|our feelings|says wrong|confusing|\bh(i|ello)\b|\bgreeting(s)?\b|error(s)?|problem(s)?|[{[(\(-]((re)?solved|edit(ed)?|updated?|fixed)[}\]\)-]|correct this|school project|beginner|promblem|asap|urgent|pl([sz]|ease)|(need|some)( halp| help| answer)|(no(t)?|doesn('?t)?) work(ing|e?d)?|waht s?|\b(h[ae]lp)\b");
		private static readonly Regex spam = new Regex(@"\b(yoga|relax(ing)?|beautiful(est)?|we lost|movers?|delhi|colon cleanse|is helpful|my huge|bulky figure|bangalore|supplements?|you know|get your|got my|six[ -]?pack)|customer reviews?|helps? your?|work out|body( ?building| ?builders?)|muscles?|weight loss|\bbrand\b|super herbal|treatment|cheaper|naturally|heal(ing|thly)?|care(ing)?|nurish(es|ing)?|ripped|full( movie| film)|free (trial|film|moive|movie|help|assistance|skin cream))\b");
