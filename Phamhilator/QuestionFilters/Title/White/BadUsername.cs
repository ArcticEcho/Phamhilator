...
namespace Phamhilator.QuestionFilters.Title.White
{
	public class BadUsername
	{
		...
		public BadUsername()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetQTWNameTermsDir()).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetQTWNameTermsDir(), site, "Terms.txt"));
		...
		{
			var file = Path.Combine(DirectoryTools.GetQTWNameTermsDir(), site, "Terms.txt");

			if (!Terms.ContainsKey(site))
			{
				Terms.Add(site, new Dictionary<Regex, float>());

				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetQTWNameTermsDir(), site));
			}
