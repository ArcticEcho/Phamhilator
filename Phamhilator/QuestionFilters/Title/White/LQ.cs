...
namespace Phamhilator.QuestionFilters.Title.White
{
	public class LQ
	{
		...
		public LQ()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetFilterFile(Filters.QuestionTitleWhiteLQ)).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetFilterFile(Filters.QuestionTitleWhiteLQ), site, "Terms.txt"));
		...
		public void AddTerm(Regex term, string site, float score = -1)
		{
			...
			if (!Terms.ContainsKey(site))
			{
				...
				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetFilterFile(Filters.QuestionTitleWhiteLQ), site));
			}
