...
namespace Phamhilator.QuestionFilters.Title.White
{
	public class Offensive
	{
		...
		public Offensive()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetQTWOffTermsDir()).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetQTWOffTermsDir(), site, "Terms.txt"));
		...
		{
			var file = Path.Combine(DirectoryTools.GetQTWOffTermsDir(), site, "Terms.txt");

			if (!Terms.ContainsKey(site))
			{
				Terms.Add(site, new Dictionary<Regex, float>());

				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetQTWOffTermsDir(), site));
			}
