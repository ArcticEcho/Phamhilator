...
namespace Phamhilator.QuestionFilters.Body.White
{
	public class Offensive
	{
		...
		public Offensive()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetQBWOffTermsDir()).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetQBWOffTermsDir(), site, "Terms.txt"));
		...
		{
			var file = Path.Combine(DirectoryTools.GetQBWOffTermsDir(), site, "Terms.txt");

			if (!Terms.ContainsKey(site))
			{
				Terms.Add(site, new Dictionary<Regex, float>());

				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetQBWOffTermsDir(), site));
			}
