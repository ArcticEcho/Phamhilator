...
namespace Phamhilator.QuestionFilters.Body.White
{
	public class LQ
	{
		...
		public LQ()
		{
			...
			var sites = Directory.EnumerateDirectories(DirectoryTools.GetFilterFile(Filters.QuestionBodyWhiteLQ)).ToArray();
			...
			{
				var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetFilterFile(Filters.QuestionBodyWhiteLQ), site, "Terms.txt"));
		...
		{
			var file = Path.Combine(DirectoryTools.GetFilterFile(Filters.QuestionBodyWhiteLQ), site, "Terms.txt");

			if (!Terms.ContainsKey(site))
			{
				Terms.Add(site, new Dictionary<Regex, float>());

				Directory.CreateDirectory(Path.Combine(DirectoryTools.GetFilterFile(Filters.QuestionBodyWhiteLQ), site));
			}
