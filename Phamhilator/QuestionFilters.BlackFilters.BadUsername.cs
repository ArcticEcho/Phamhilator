	...
	namespace QuestionFilters
	{
		namespace BlackFilters
		{
			public class BadUsername
			{
				...
				public BadUsername()
				{
					var data = File.ReadAllLines(DirectoryTools.GetQBlackBadUsernameTermsFile());
				...
				public void AddTerm(Regex term)
				{
					...
					File.AppendAllText(DirectoryTools.GetQBlackBadUsernameTermsFile(), "\n" + AverageScore + "]" + term);
				}
				...
				public void RemoveTerm(Regex term)
				{
					...
					var data = File.ReadAllLines(DirectoryTools.GetQBlackBadUsernameTermsFile()).ToList();
					...
					File.WriteAllLines(DirectoryTools.GetQBlackBadUsernameTermsFile(), data);
				}
