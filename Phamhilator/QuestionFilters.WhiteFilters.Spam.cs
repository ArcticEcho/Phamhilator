		...
		namespace WhiteFilters
		{
			public class Spam
			{
				...
				public Spam()
				{
					...
					var sites = Directory.EnumerateDirectories(DirectoryTools.GetQWhiteSpamTermsDir()).ToArray();
					...
					{
						var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetQWhiteSpamTermsDir(), site, "Terms.txt"));
				...
				{
					var file = Path.Combine(DirectoryTools.GetQWhiteSpamTermsDir(), site, "Terms.txt");

					if (!Terms.ContainsKey(site))
					{
						Terms.Add(site, new Dictionary<Regex, float>());

						Directory.CreateDirectory(Path.Combine(DirectoryTools.GetQWhiteSpamTermsDir(), site));
					}
