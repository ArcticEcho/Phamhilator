	...
	namespace WhiteFilters
	{
		public class BadUsername
		{
			...
			{			
				...
				var sites = Directory.EnumerateDirectories(DirectoryTools.GetWhiteBadUsernameTermsDir()).ToArray();
				...
				{
					var data = File.ReadAllLines(Path.Combine(DirectoryTools.GetWhiteBadUsernameTermsDir(), site, "Terms.txt"));
			...
			{
				var file = Path.Combine(DirectoryTools.GetWhiteBadUsernameTermsDir(), site, "Terms.txt");

				if (!Terms.ContainsKey(site))
				{
					Terms.Add(site, new Dictionary<Regex, int>());

					Directory.CreateDirectory(Path.Combine(DirectoryTools.GetWhiteBadUsernameTermsDir(), site));
				}
